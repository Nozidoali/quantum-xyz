...
Last Modified time: 2023-06-25 17:46:49
'''
...
class CnRyDataBaseEntryPoint:
    ...
    def canonicalize(self) -> 'CnRyDataBaseEntryPoint':
        ...
        """
        num_qubits = int(np.ceil(np.log2(float(max(self.states)+1)))) if max(self.states) > 0 else 0
        state, transitions = to_canonical_state(self.states, num_qubits)
        moves = zip(*transitions)
        return CnRyDataBaseEntryPoint(state), moves

class CnRyDateBaseNode:

    def __init__(self, key: CnRyDataBaseEntryPoint, prev: 'CnRyDateBaseNode', move_from_prev: CnRyMove, canonical_transitions: List[CnRyMove], cost: int) -> None:
        self.key = key
        self.prev = prev
...
class CnRyDataBase:
    ...
    def initialize(self, target_entry_point: CnRyDataBaseEntryPoint = None):
        ...
        while not q.empty():
            ...
            for move in possible_moves:
                ...
                canonicalized_entry_point, moves = next_entry_point.canonicalize()
                ...
                if target_entry_point is not None:
                    ...
                    if canonicalized_entry_point == target_entry_point:
                        if terminate_cost is None or new_cost < terminate_cost:
                            ...
                            terminate_node = CnRyDateBaseNode(canonicalized_entry_point, curr_node, move, moves, terminate_cost)
                ...
                # create a new node
                new_node = CnRyDateBaseNode(canonicalized_entry_point, curr_node, move, moves, new_cost)
    ...
    def lookup(self, state: List[int]):
        ...
        """
        entry_point, preprocessing = CnRyDataBaseEntryPoint(state).canonicalize()
        ...
        if entry_point not in self.look_up_table:
            ...
            while curr_node != None:
                ...
                moves = curr_node.canonical_transitions
                if prev_node is not None:
                    print(f"state: {curr_node.key}, cost = {move.cost()}")
                curr_node = prev_node