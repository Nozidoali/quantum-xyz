...
Last Modified time: 2023-06-25 18:02:21
"""
...
def solution_to_circuit(num_qubits: int, solution: list) -> QCircuit:
    ...
    for step in solution:
        ...
        for i in range(1 << num_qubits):
            ...
            if direction == CnRYDirection.MERGE:
                ...
                curr_theta = None
                try:
                    curr_theta = 2 * np.arccos(
                        np.sqrt(
                            weights[dst_index]
                            / (weights[src_index] + weights[dst_index])
                        )
                    )
            ...
            if len(thetas) == 0:
                ...
                if len(thetas) == 2:
                    ...
                    if len(control_qubits) == 1:
                        # special case
                        gate = MULTIPLEXY(
                            theta1, theta2, control_qubits[0], circuit.qubit_at(qubit)
                        )
                        mcry_gates.append(gate)

                    else:
                        gate1 = MCRY(
                            theta1, control_qubits, phases1, circuit.qubit_at(qubit)
                        )
                        gate2 = MCRY(
                            theta2, control_qubits, phases2, circuit.qubit_at(qubit)
                        )
    ...
    for gate in mcry_gates[::-1]:
        ...
        if circuit.has_mcry():
            ...
            control_qubits = [qubit for qubit, _ in control_qubits]
