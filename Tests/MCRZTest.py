...
Last Modified time: 2023-06-18 22:54:11
"""
...
def test_mcrz():
    ...
    matrix = np.kron(
        np.identity(2), np.kron(c1, d1) + np.kron(c2, d1) + np.kron(c1, d2)
    ) + np.kron(u1, np.kron(c2, d2))

    half = matrix.shape[0] // 2
    S = matrix[:half, :half]

    alphas = -2 * np.imag(np.log(S.diagonal()))
    ...
    for i, theta in enumerate(thetas):
        ...
        new_matrix = new_matrix @ np.kron(
            BasicGate.rz(theta), np.identity(2 ** (num_qubits - 1))
        )
        # print("new matrix = \n", new_matrix)
