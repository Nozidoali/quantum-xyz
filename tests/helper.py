...
BEST_CNOT_RESULT_FILE = os.path.join(
    os.path.dirname(__file__), "best_cnot_results", "best_cnot_results.json"
)
BEST_CNOT_QASM_FOLDER = os.path.join(os.path.dirname(__file__), "best_cnot_results")
BEST_TIME_RESULT_FILE = os.path.join(
    os.path.dirname(__file__), "best_time_results", "best_time_results.json"
)
...
def run_experiment(
    ...
    print(
        f"Running {filename}... state density = {len(state.index_to_weight)}, num_qubits = {state.num_qubits},"
    )
    ...
    with stopwatch("synthesis") as timer:
        ...
        if num_cnots != num_cnots_qiskit:
            print_yellow(
                f"The number of CNOT gates is {num_cnots}, but the number of CNOT gates in Qiskit is {num_cnots_qiskit}"
            )
    ...
    try:
        _, _, cx_merge = run_sparse_state_synthesis(state.to_vector())
    except:
        cx_merge = None

    try:
        cx_sparse_uniform = run_dd_based_method(state.to_vector())
    except:
    ...
    new_results[filename] = {
        ...
        "num_cnots_merge": cx_merge,
        "num_cnots_sparse_uniform": cx_sparse_uniform,
    }
...
def compare_and_update_results(
    best_result_filename: str, new_results: dict, best_qasm_dir, qasms: dict, key: str
):
...
def update_results(new_results: dict):
    ...
    for best_result_filename, qasm_dir, key in zip(
        ...
        compare_and_update_results(
            best_result_filename, new_results_stats, qasm_dir, qasms, key
        )
