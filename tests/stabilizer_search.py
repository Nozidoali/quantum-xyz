...
class StabilizerState:
    def __init__(self, num_qubits: int, num_stabilizers: int, patterns: List[int]):
        ...
        if PERM_INVARIANT: self.regularize()
    ...
    def is_final(self) -> bool:
        for i, p in enumerate(self.patterns):
            if p != (1<<(self.num_stabilizers-i-1)): return False
            if i == self.num_stabilizers: break
        return True
    ...
    def apply_cx(self, cx: dict) -> 'StabilizerState':
        next_state = StabilizerState(self.num_qubits, self.num_stabilizers, self.patterns)
        next_state.patterns[cx['target']] ^= next_state.patterns[cx['control']]
        if PERM_INVARIANT: next_state.regularize()
        return next_state
    ...
    while q.qsize() > 0:
        ...
        if state.is_final():
            ...
            for s in path:
                print(s)
            break
        if hash(state) in visited: continue
        ...
        for i in range(state.num_qubits):
            for j in range(state.num_qubits):
                ...
                if hash(next_state) in visited or next_state in enqueued and enqueued[next_state] <= cost + 1: continue
