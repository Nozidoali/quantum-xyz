...
def test_synthesis(num_qubit: int):
    ...
    for sparsity in range(1, 2**num_qubit):
        ...
        # for state in all_states(num_qubit, sparsity):
            ...
            print(f"num_qubit = {num_qubit} sparsity = {sparsity} state = {state} baseline = {baseline} ours = {ours}, time = {timer.time()}")
            
            data = {"num_qubit": num_qubit, "sparsity": sparsity, "baseline": baseline, "ours": ours, "time": timer.time()}
            datas.append(data)
...
def analyze_runtime(num_qubit: int):
    ...
    sns.scatterplot(data=df, x="ours", y="time", hue="sparsity", palette="rocket_r", markers=True, alpha=0.8, s=300)
...
def analyze_data(num_qubit: int):
    ...
    df_agg = df.groupby(["sparsity"]).agg({"baseline": "mean", "ours": "mean", "time": "mean"})
    # calculate the standard deviation
    df_agg["num_cnot_baseline_std"] = df.groupby(["sparsity"]).agg({"baseline": "std"})["baseline"]
    ...
    # data points
    if False:
        plt.scatter(df["sparsity"] - 2 - 0.13, df["baseline"], marker="x", color="black", alpha=0.2)
        plt.scatter(df["sparsity"] - 2 + 0.13, df["ours"], marker="x", color="black", alpha=0.2)
    
    # data labels of the mean 
    data_label_font_size = global_font_size
    precision = 0
    for index, row in df_agg.iterrows():
        plt.text(index - 2 - 0.11, row["baseline"] + 10 + 0.1, f"{row['baseline']:.{precision}f}", color="black", fontsize=data_label_font_size)
        plt.text(index - 2 + 0.15, row["ours"] + 0.1, f"{row['ours']:.{precision}f}", color="black", fontsize=data_label_font_size)
    
    # with an offset
    plt.errorbar(df_agg.index - 2 - 0.13, df_agg["baseline"], yerr=df_agg["num_cnot_baseline_std"], fmt='none', ecolor='black', capsize=5)
    plt.errorbar(df_agg.index - 2 + 0.13, df_agg["ours"], yerr=df_agg["num_cnot_std"], fmt='none', ecolor='black', capsize=5)
