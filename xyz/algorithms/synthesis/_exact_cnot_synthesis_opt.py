...
def exact_cnot_synthesis_opt(
    ...
    def explore_state(
        ...
        for gate in gates:
            try:
                match gate.qgate_type:
                    ...
                    case qc.QGateType.CX:
                        ...
                        next_state = next_state.apply_cx(
                            gate.control_qubit.index, gate.phase, gate.target_qubit.index
                        )
                        gate_cost += 1
                ...
                if verbose_level >= 3:
                    print(f"next_state: {next_state}, curr_state = {curr_state} gate = {gate}")
            except ValueError:
                return None
        ...
        if verbose_level >= 3:
            print(f"recording [{hash(next_state)}] <- {hash(curr_state)}, gate: {gates}")
    ...
    while not state_queue.empty():
        ...
        if verbose_level == 1:
            print(f"#visited: {len(visited_states)} #enquened: {state_queue.qsize()}, #cost: {curr_cost}\r", end="")
        ...
        for target_qubit in supports:
            ...
            for theta in sorted(thetas):
                explore_state(curr_state, [qc.RY(theta, circuit.qubit_at(target_qubit))], curr_cost)
                continue
                    ...
                    for theta in sorted(cry_thetas):
                        ...
                        explore_state(
                            ...
                            [
                                ...
                                qc.CX(circuit.qubit_at(control_qubit), 0, circuit.qubit_at(target_qubit)),
                                qc.RY(-theta, circuit.qubit_at(target_qubit)),
                            ],
                            curr_cost,
                        )
                        explore_state(
                            ...
                            [
                                qc.RY(theta, circuit.qubit_at(target_qubit)),
                                qc.CX(circuit.qubit_at(control_qubit), 1, circuit.qubit_at(target_qubit)),
                                qc.RY(-theta, circuit.qubit_at(target_qubit)),
                            ],
                            curr_cost,
                        )
