...
def explore_state(
    srg: SRGraph, curr_state: QState, operator: QOperator, curr_cost: int
) -> None:
    ...
    next_cost = (
        curr_cost
        + operator.get_cost()
        + srg.get_lower_bound(next_state)
    ...
    for i in y_signatures:
        ...
        srg.record[state] = next_state, MCRYOperator(
            i, QuantizedRotationType.MERGE0, [], []
        )
        next_state = state
...
def synthesize_srg(target_state: QState, verbose: bool = False) -> SRGraph:
    ...
    while not srg.search_done():
        ...
        if verbose:
            print(
                f"cost = {curr_cost}, visited = {len(srg.visited_states)}, in record: {len(srg.record)}",
                end="\r",
            )
            sys.stdout.flush()
